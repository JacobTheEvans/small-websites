...
app.controller("mainControler",["$scope", "$timeout", function($scope,$timeout) {
  $scope.userInput = {};
  $scope.dataToPlot = [
  ];
  $scope.doSort = false;
  ...
  $scope.add = function() {
    $scope.dataToPlot.push({key: $scope.userInput.key, value: $scope.userInput.value});
    $scope.userInput.key = "";
    $scope.userInput.value = "";
  };
  $scope.remove = function($index) {
    $scope.dataToPlot.splice($index,1);
  };
  $scope.save = function() {
    console.log("SVG");
    console.log(document.getElementById("chart-save"));
    saveSvgAsPng(document.getElementById("chart-save"), "bargraph.png");
  };
